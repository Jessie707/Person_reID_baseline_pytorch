...
def weights_init_kaiming(m):
    ...
    if classname.find("Conv") != -1:
        ...
        )  # 将BatchNorm1d的参数w初始化：服从均值为1，方差为0.02的正态分布
    if hasattr(m, "bias") and m.bias is not None:
        init.constant_(m.bias.data, 0.0)  # 将参数b初始化为0
...
class ClassBlock(nn.Module):
    def __init__(
        ...
        add_block = nn.Sequential(
            *add_block
        )  # 此处*的作用：将列表add_block中的元素拆成一个个单独的元素 参考来源:https://blog.csdn.net/u013548568/article/details/80294708 中3python*号
        add_block.apply(
            weights_init_kaiming
        )  # 递归地将weights_init_kaiming函数（即apply括号内的函数）应用于add_block及其子模块
...
class ft_net18(nn.Module):
    def __init__(
        ...
        self.circle = circle
        self.classifier = ClassBlock(
            512, class_num, droprate, linear=linear_num, return_f=circle
        )
