...
parser.add_argument(
    "--which_epoch", default="last", type=str, help="0,1,2,3...or last"
)  # 注意
parser.add_argument(
    "--test_dir",
    default="/home/ljl/Data/REID/ORS_REID/ORS_REID_DVD",
    type=str,
    help="./test_data",
)  # 需要修改
# parser.add_argument(
...
parser.add_argument(
    "--name", default="ft_ResNet50", type=str, help="save model path"
)  # 注意
...
parser.add_argument("--multi", action="store_true", help="use multiple query")  # 可以利用
...
config_path = os.path.join(
    "./model", opt.name, "opts.yaml"
)  # opt.name与train.py中保存的名称一致
...
def load_network(network):
    save_path = os.path.join(
        "./model", name, "net_%s.pth" % opt.which_epoch
    )  # 与train.py保存模型保持一致
